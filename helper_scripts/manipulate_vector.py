...
parser.add_argument("--plot_tiles", dest = 'plot', action = 'store_true', help = "Flag to indicate that features should be plotted in relation to Sentinel-2 tiles.")
parser.add_argument("--simplify_tolerance", dest = 'tolerance', type = float, default = None, help = "Tolerance value for simplifying geometries.")
parser.add_argument("--simplify_topology", dest = 'topology', type = bool, default = True, help = "True/False to indicate if feature topology should be preserved during simplifying. Defaults to True. Requires --simplify_tolerance.")
...
def check_validity(vectorfile):
    ...
    vectorfile_with_geom = vectorfile_with_geom.loc[vectorfile_with_geom['validity'] == False]
    # If invalid geometries exist, run explain_validity for them
    if len(vectorfile_with_geom) > 0:
        vectorfile_with_geom['explanation'] = vectorfile_with_geom.apply(lambda row: explain_validity(row.geometry), axis = 1)
...
def remove_fields(vectorfile, fields):
    ...
    try:       
        ...
        if (type(value) == int) and (value > len(vectorfile.columns)):
            exit("ERROR: Given index exceeds the number of columns in vectorfile. Please check your input.")
        else:
    ...
    for field in field_names:
        if not field in vectorfile.columns:
            exit("ERROR: The field {} was not found in {}, please check your input.".format(field, args.vector))
...
def plot_tiles(vectorfile):
    ...
    sen2tiles = gpd.read_file(os.path.join(parent_dir, 'src', 'sentinel2_tiles_world', 'sentinel2_tiles_world.shp'))
