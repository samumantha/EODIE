...
class SplitshpObject(object):
    ...
    def __init__(
        ...
        self.small_polygon_shapefile = self.reproject_to_epsg(
            small_polygon_shapefile, "4326"
        )
        self.large_polygon_shapefile = self.reproject_to_epsg(
            large_polygon_shapefile, "4326"
        )
        self.test = test
    ...
    def reproject_to_epsg(self, myshp, myepsg):
        ...
        if epsgcode == myepsg:
            ...
            reprojectedshape = os.path.join(
                self.output_directory, root + "_reprojected_" + myepsg + ext
            )
            if not os.path.exists(reprojectedshape):
                ...
                logging.info(
                    "input shapefile had other than EPSG "
                    + myepsg
                    + " but was reprojected and works now"
                )
    ...
    def write_splitted_shapefiles(
        self, bounding_box_small_poly_shp, tile, outshpdir, small_poly_shp
    ):
        ...
        """
        one_tile_geometry = self.make_geometryobject(
            tile["geometry"]
        )  # on puhti: TypeError: 'NoneType' object is not subscriptable
        if one_tile_geometry.intersects(bounding_box_small_poly_shp):
            ...
            outshpname = os.path.join(
                outshpdir, originalname + "_" + str(tilename) + ".shp"
            )
                ...
                with fiona.open(outshpname, "w", driver, schema, crs) as outputshp:
                    for small_polygon in smaller_open_shp:
                        ...
                        if not small_polygon_geometry is None:
                            ...
                            if self.make_geometryobject(
                                small_polygon_geometry
                            ).intersects(one_tile_geometry):
                                outputshp.write(
                                    {
    ...
    def splitshp(self):
        ...
        for tile in self.tiles:
            exists = os.path.exists(
                os.path.join(self.output_directory, root + "_" + tile + ".shp")
            )
            if not exists:
                break
        if not exists:
        ...
        removelist = glob.glob(
            os.path.join(self.output_directory, largeroot + "_" + root + ".*")
        )
    ...
    def splitshp_world(self):
        ...
        self.out_shape_name = os.path.join(
            self.output_directory,
            os.path.splitext(os.path.split(self.large_polygon_shapefile)[-1])[0]
            + "_"
            + os.path.split(self.small_polygon_shapefile)[-1],
        )

        self.extract_needed_tiles(
            self.small_polygon_shapefile,
            self.large_polygon_shapefile,
            self.out_shape_name,
        )
    ...
    def splitshp_mp(self):
        ...
        self.large_polygon_shapefile = self.reproject_to_epsg(
            self.out_shape_name, "4326"
        )

        bounding_box_small_poly_shp = self.get_bounding_box(
            self.small_polygon_shapefile
        )
        ...
        logging.info(
            "number of usable cores for shapesplitting is "
            + str(usable_number_of_cores)
        )
        ...
        with fiona.open(self.large_polygon_shapefile, "r") as s2shp:
            for tile in s2shp:
                pool.apply_async(
                    self.write_splitted_shapefiles,
                    args=(
                        bounding_box_small_poly_shp,
                        tile,
                        self.output_directory,
                        self.small_polygon_shapefile,
                    ),
