...
class TileSplitter(object):
    ...
    def reproject_to_epsg(self, vectorfile ,myepsg):
        ...
        head, tail = os.path.split(vectorfile)
        ...
        if epsgcode == myepsg:
            logging.info('input vectorfile has EPSG '  +myepsg + ' that works!')
            return vectorfile 
        else:
            ...
            if not os.path.exists(reprojectedvectorfile):
                reprojectcommand = 'ogr2ogr -t_srs EPSG:' + myepsg + ' ' +  reprojectedvectorfile + ' ' + vectorfile 
                #print(reprojectcommand)
                subprocess.call(reprojectcommand, shell=True)
                logging.info('input vectorfile had other than EPSG ' + myepsg + ' but was reprojected and works now')
