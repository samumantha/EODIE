    ...
    if entry.name.endswith('.csv') and entry.is_file: #Makes sure it is a correct kind of file
        with open(entry,'r',newline='') as file: 
            ...
            if not stat_list.__contains__(titles):
                os.mkdir(outputDest+'csvFiles_with_'+'_'.join(titles)) #if the directory doesn't exist, it creates one
                stat_list.append(titles)
               
                
            command='cp '+entry.path + ' ' +outputDest+'/csvFiles_with_'+'_'.join(titles)
            os.system(command) #Executes the command in the system, copies the file

print('The files should now be copied in separate directories based on their statistics')
