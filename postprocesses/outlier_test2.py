...
parser.add_argument('--dir', dest= 'directory' , default="/home/jvarho/EODIE/testTimeseries/combined_kndvi.csv",help = 'Write the directory path where the csv files are as absolute path')
...
parser.add_argument('--id', dest='ID', default="63,32,1,3" ,help="Write a list of id's separated by a comma")
parser.add_argument('--index', dest= 'index', default="kndvi,ndvi,rvi,savi" ,help='write a list of indices you want to be plotted separated by a comma')
...
parser.add_argument('--files',dest='fileFormat',default='png', help="write the wanted format of timeseries, can be: 'png','eps','pdf','ps','svg'")
...
if '.csv' in directory and 'combined_' in directory.split('/')[-1]: #If input is one combined statistics file
    ...
    for entry in os.scandir(directory): #checks every file in the directory given as input
        if(entry.name.endswith('.csv') and entry.is_file and indices_list.__contains__(entry.name.split('_')[0])):  #Makes sure the file to be read is .csv file, is the correct index and is a file to begin with
            ...
            temp_df=pd.read_csv(entry).query("id in @test_ids")# and mean!= 'None'"
            temp_df['Dates']=filedate # Change to filedate if datetime object wanted
            temp_df['Index']=VI
...
for id in ID_list:
    for index in indices_list:
        #plot_df=mainDataFrame.query("id==@id and Index==@index")[wanted_data]

        plot_df=mainDataFrame.query("id==@id and Index==@index and mean!='None'").set_index('Dates')#Drops unused data columns

        if plot_df.empty: #If after query the df is empty, skips the iteration
            continue      
