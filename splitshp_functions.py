...
def write_splitted_shapefiles(bounding_box_small_poly_shp,tile,outshpdir, small_poly_shp):
    ...
    one_tile_geometry = make_geometryobject(get_parameter_content(tile,'geometry')) #on puhti: TypeError: 'NoneType' object is not subscriptable
    ...
    if one_tile_geometry.intersects(bounding_box_small_poly_shp):
        ...
        print(originalname)
        if 'reprojected' in originalname:
            originalname = os.path.split(originalname)[-1].split('_reprojected')[0]
        print(originalname)
